    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 p-[20px] bg-[#F3F4F6]">
      {features.map((feature, index) => (
        <div key={index} className="flex items-start gap-4">
          {/* Icon Circle */}
          <div className="flex-shrink-0 w-[34px] h-[34px] bg-[#111111] rounded-full flex items-center justify-center">
            {feature.icon}
          </div>
          
          {/* Text Content */}
          <div className="flex flex-col gap-[4px]">
            <h1 className="text-lg font-medium text-[#111111] leading-tight">
              {feature.title}
            </h1>
